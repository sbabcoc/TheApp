...
async function requestLocationPermission() {
  if (Platform.OS === "android") {
    ...
    Navigation.geolocation.requestAuthorization();
  }
}
...
export default class LocationScreen extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      Navigation.geolocation.getCurrentPosition(onSuccess, onErr);
      this.watchId = Navigation.geolocation.watchPosition(onSuccess, onErr);
    } catch (err) {
  ...
  componentWillUnmount() {
    Navigation.geolocation.clearWatch(this.watchId);
  }
